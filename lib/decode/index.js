        ...
        init: function(trama, ip, port) {
            ...
            return {
                database: function() {
                    ...
                    var utctime = moment(this.utcTimeYear()+'-'+this.utcTimeMonth()+'-'+this.utcTimeDay()+' '+this.utcTimeHours()+':'+this.utcTimeMinutes()+':'+this.utcTimeSeconds());
                        utctime = utctime.isValid() ? utctime.format(CUSTOM_DATE.dateTimeFormat) : null;
                    var celldatetime = moment(utctime);
                    ...
                    var gpsdatetime = moment(this.utcTimeYear()+'-'+this.utcTimeMonth()+'-'+this.lastGpsFix().dayOfMonth()+' '+this.lastGpsFix().hours()+':'+this.lastGpsFix().minutes()+':'+this.utcTimeSeconds());
                ...
                unitsHardwareVersion: function() {
                    ...
                    for (var property in HARDWARE){
                        if ( HARDWARE[property].modeloId == _modelo && HARDWARE[property].modemCode == _modem ) {
                            _version = property;
                        }
                    }
