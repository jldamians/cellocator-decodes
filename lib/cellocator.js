...
var Cellocator = function(trama, ip, port){
    this.trama = trama || '' ;
    this.port  = port || '3000' ;
...
Cellocator.prototype.database = function() {
    ...
    var _insertdate     = moment(new Date(), DATE_TIME_FORMAT).toDate();
    var _gpsdatetime    = _insertdate.getFullYear()+'-'+(_insertdate.getMonth() + 1)+'-'+this.lastGpsFix().dayOfMonth()+' '+this.lastGpsFix().hours()+':'+this.lastGpsFix().minutes()+':00'; 
        _gpsdatetime    = moment(_gpsdatetime, DATE_TIME_FORMAT).toDate();
