...
function Cellocator (trama, ip, port){
    // change the global locale to Spanish
...
(function() {
    this.database = function() {
        var _rmuid          = util.hexToDec(this.unitsId());
        var _utctime        = moment(this.utcTimeYear()+'-'+this.utcTimeMonth()+'-'+this.utcTimeDay()+' '+this.utcTimeHours()+':'+this.utcTimeMinutes()+':'+this.utcTimeSeconds(), constants.DATE_TIME_FORMAT);
            _utctime        = _utctime.isValid() ? _utctime.format(constants.DATE_TIME_FORMAT) : null;
        var _celldatetime   = moment(_utctime).add(constants.GREENWICH, 'seconds').format(constants.DATE_TIME_FORMAT);
        var _gpsx           = this.longitudeAndLatitude().latitude(); 
        var _gpsy           = this.longitudeAndLatitude().longitude(); 
        var _insertdate     = this.now.format(constants.DATE_TIME_FORMAT);
        var _gpsdatetime    = moment(this.now.year()+'-'+this.now.month()+'-'+this.lastGpsFix().dayOfMonth()+' '+this.lastGpsFix().hours()+':'+this.lastGpsFix().minutes()+':'+this.utcTimeSeconds(), constants.DATE_TIME_FORMAT); 
            _gpsdatetime    = _gpsdatetime.isValid() ? _gpsdatetime.format(constants.DATE_TIME_FORMAT) : null;
        var _speed          = this.groundSpeed(); 
        var _direction      = this.speedDirection(); 
        var _numofsat       = this.numberOfSatellitesUsed();
        var _locquality     = -1; 
        var _engineon       = util.hexToDec(this.unitsModeOfOperation()); 
        var _extinputa      = util.binToDec(this.unitsInputOutputStatus2stByte().lock()); 
        var _extinputb      = util.binToDec(this.unitsInputOutputStatus1stByte().unlockInactive()); 
        var _extinputc      = util.binToDec(this.unitsInputOutputStatus2stByte().ignitionPortStatus());
        var _extinputd      = util.binToDec(this.unitsInputOutputStatus1stByte().doorInactive());
        var _extinpute      = util.binToDec(this.unitsInputOutputStatus1stByte().shockInactive());
        var _extinputf      = util.binToDec(this.unitsInputOutputStatus1stByte().panicInactive());
        ...
        var _alertreason    = -1;
        var _hibernation    = util.binToDec(this.communicationControlField().noHibernation());
        var _hrnetwork      = -1;
        var _milecounter    = this.mileageCounter(); 
        var _gpsmode1       = util.binToDec(this.mode1());
        var _gpsmode2       = util.binToDec(this.mode2());
        var _outputa        = util.binToDec(this.unitsInputOutputStatus3stByte().gradualStopInactive());
        var _outputb        = -1;
        var _outputc        = util.binToDec(this.unitsInputOutputStatus4stByte().standardImmobilizer());
        var _outputd        = util.binToDec(this.unitsInputOutputStatus3stByte().sirenInactive()); 
    ...
    this.unitsHardwareVersion = function() {
        ...
        _modem  = util.binToDec(byte13.substr(0, 3)) ;
        _modelo = util.binToDec(byte13.substr(3, 5)) ;
    ...
    this.unitsInputOutputStatus4stByte = function() {
        ...
        return {
            ...
            plmn: function() {
                return util.hexToDec(self.unitsStatusCurrentGsmOperator().currentGsmOperator1stNibble()+''+self.currentGsmOperator()+''+byte25) ;
            }
        }
    }
    ...
    this.lastGpsFix = function() {
        ...
        return {
            dayOfMonth: function() {
                return util.binToDec(bytefrom39to40.substr(0, 5)) ;
            },
            hours: function() {
                return util.binToDec(bytefrom39to40.substr(5, 5)) ;
            },
            minutes: function() {
                return util.binToDec(bytefrom39to40.substr(10, 6)) ;
            }
        }
    }
...
}).call(Cellocator.prototype)

exports.Cellocator = Cellocator;