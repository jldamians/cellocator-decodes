...
Cellocator.prototype.database = function() {
    ...
    _utctime = moment(_utctime, DATE_TIME_FORMAT).toDate() ;
    var _celldatetime = addSubtractDate.add(_utctime, GREENWICH, "seconds") ;
    ...
    return {
        RMUId: this.getRMUId(), // Integer
        CellDateTime: this.getCellDateTime(), // DateTime
        GPSX: this.getGPSX(), // Integer
        GPSY: this.getGPSY(), // Integer
        GPSDateTime: this.getGPSDateTime(), // DateTime
        Speed: this.getSpeed(), // Integer
        Direction: this.getDirection(), // Integer
        NumOfSat: this.getNumOfSat(), // Integer
        LocQuality: -1, // Integer
        EngineOn: this.getEngineOn(), // Integer
        ExtInputA: this.getExtInputA(), // Integer
        ExtInputB: this.getExtInputB(), // Integer
        ExtInputC: this.getExtInputC(), // Integer
        ExtInputD: this.getExtInputD(), // Integer
        ExtInputE: this.getExtInputE(), // Integer
        ExtInputF: this.getExtInputF(), // Integer
        VersionNum: this.getVersionNum(), // Varchar(25)
        IP: _socket.ip, // Char(15)
        InputVoltage: this.getInputVoltage(), // Integer
        BackBatVoltage: this.getBackBatVoltage(), // Integer
        GPSPDOP: -1, // Small Integer(2 bytes)
        GPSHEIGHT: this.getGPSHEIGHT(), // Integer
        NetworkTypeId: 0, // Small Integer(2 bytes)
        InsertDate: _localDateTime, // DateTime
        UTCTime: this.getUTCTime(), // DateTime
        DriverId: '-1', // Long(8 bytes)
        TxReasonId: this.getTxReasonId(), // Integer
        AlertReason: -1, // Integer
        Hibernation: -1, // Integer
        HRNetwork: -1, // Integer
        MileCounter: this.getMileCounter(), // Integer
        GPSMode1: this.getGPSMode1(), // Integer
        GPSMode2: this.getGPSMode2(), // Integer
        OutputA: this.getOutputA(), // Integer
        OutputB: -1, // Integer
        OutputC: this.getOutputC(), // Integer
        OutputD: this.getOutputD(), // Integer
        OptionalInput: -1, // Integer
        GPSCommStatus: this.getGPSCommStatus(), // Integer
        RawData: _trama, // Varchar(2000)
        PLMN: this.getPLMN(), // Integer
        SN: this.getSN(), // Integer
        MessageType: this.getMessageType(), // Integer
        MsgProtocol: -1, // Small Integer(2 bytes)
        TripId: -1, // Integer
        ManeuverId: -1, // Integer
        ManeuverUsage: -1, // Small Integer(2 bytes)
        AccidentBuffer: -1, // Small Integer(2 bytes)
        ItemId: '-1' // Long
    }
}
